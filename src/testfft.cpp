...
void testfft(hls::stream<trans_pkt>& src, hls::stream<trans_pkt>& dst)
{
...
#pragma HLS pipeline
		...
		LineBuffer[i] = (DTYPE)data_p.data;
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=128
	...
	fft_streaming(LineBuffer,R,RO,IO);
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=128
#pragma HLS pipeline
		data_p.data = sqrt(RO[i]*RO[i]+IO[i]*IO[i]);
		dst.write(data_p);
